...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, ref):
        ...
        self.key_value_slicing = (self.num_key_value_heads * self.head_dim) // self.config.pretraining_tp
        self.query_slices = self.q_proj.weight.split((self.num_heads * self.head_dim) // self.config.pretraining_tp, dim=0)[rank]
        self.key_slices = self.k_proj.weight.split(self.key_value_slicing, dim=0)[rank]
        self.value_slices = self.v_proj.weight.split(self.key_value_slicing, dim=0)[rank]

        hidden_size = self.hidden_size // self.config.pretraining_tp
        self.o_proj_slices = self.o_proj.weight.split(hidden_size, dim=1)[rank]
    ...
    def forward(
        ...
        if self.config.pretraining_tp > 1:
            ...
            query_states = F.linear(hidden_states, self.query_slices)
            key_states = F.linear(hidden_states, self.key_slices)
            value_states = F.linear(hidden_states, self.value_slices)
        ...
        if self.config.pretraining_tp > 1:
            ...
            attn_output = F.linear(attn_output, self.o_proj_slices)
